...
$ (function () {
	...
	if (canvas && canvas.getContext) {
		...
		background.canvasBG = document.createElement ('canvas');
		background.canvasBG.width = canvas.width;
		background.canvasBG.height = canvas.height;
		background.ctxBG = background.canvasBG.getContext ('2d');
		background.noOfSquares = background.canvasBG.width * background.canvasBG.height / 10;
		...
		*/
		...
		game.drawBackground = function () {
			...
			console.log(bgItem);
			game.draw(bgItem, ctx, function (done, img) {
				for (var x=0; x<(background.canvasBG.width/10); x++) {
					for (var y=0; y<(background.canvasBG.height/10); y++){
						background.ctxBG.drawImage(img, x*10,y*10);		
					}
				}
			});
		}

		game.draw = function (item, context, cb) {
			console.log('item.type:', item.type);
			if (game.images[item.type][item.name] === undefined) {
				var img = game.images[item.type][item.name] = game.loadImg(item);//game.currentPlayer.units[0]);
				...
				context.drawImage(game.images[item.type][item.name],item.x,item.y); // Draws an individual item onto the context buffer
		...
		game.drawItems = function (items) {
			...
			for (var i=0; i<items.length; i++) {
				...
				game.draw (items[i], ctxBuffer, function (bLoaded) {
					iNoLoaded+=1;
					if (iNoLoaded === items.length) {
						game.clear(ctx); // Clear the context
						ctx.drawImage(background.canvasBG,0,0);
